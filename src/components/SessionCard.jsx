    ...
    <div className={`session-card ${expanded ? 'expanded' : ''} status-${session.status}`}>
      <div className="card-header" onClick={() => setExpanded(!expanded)}>
        ...
        <div className="card-header-right">
          ...
          <button className="btn-qr" onClick={(e) => { e.stopPropagation(); onShowQR(session); }} title="微信扫码连接">
          ...
          <button className="btn-expand" onClick={(e) => { e.stopPropagation(); setExpanded(!expanded); }} title={expanded ? '收起' : '展开'}>
      ...
      {expanded && (
        <div className="card-body">
          ...
          <div className="card-input">
            <input type="text" className="msg-input" placeholder={isActive ? '输入指令发送到 Claude...' : '会话已结束'}
              value={inputValue} onChange={(e) => setInputValue(e.target.value)}
              onKeyDown={handleKeyDown} disabled={!isActive || isSending} />
      ...
      <style>{`
        .session-card {
          ...
          transition: all var(--transition); animation: slide-in 0.3s ease-out;
        ...
        .session-name { font-size: 13px; font-weight: 600; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .session-cwd { font-size: 10px; color: var(--text-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        ...
        .card-body { padding: 0 14px 12px; animation: slide-in 0.25s ease-out; -webkit-app-region: no-drag; }
        .message-list { max-height: 200px; overflow-y: auto; margin-bottom: 10px; display: flex; flex-direction: column; gap: 6px; }
        ...
        .msg-role { font-size: 9px; font-weight: 600; text-transform: uppercase; color: var(--text-muted); }
        .msg-content { color: var(--text-primary); }
        .msg-time { font-size: 9px; color: var(--text-muted); text-align: right; }
        .card-input { display: flex; gap: 6px; -webkit-app-region: no-drag; }
        .msg-input { flex: 1; background: rgba(255,255,255,0.05); border: 1px solid var(--border-subtle); border-radius: var(--radius-sm); padding: 7px 10px; font-size: 12px; color: var(--text-primary); outline: none; transition: border-color 0.2s; -webkit-app-region: no-drag; }
        .msg-input:focus { border-color: var(--border-active); }
        .msg-input:disabled { opacity: 0.4; }
        .btn-send { background: var(--accent); border: none; border-radius: var(--radius-sm); padding: 7px 14px; font-size: 12px; font-weight: 600; color: white; cursor: pointer; transition: all 0.2s; white-space: nowrap; -webkit-app-region: no-drag; }
